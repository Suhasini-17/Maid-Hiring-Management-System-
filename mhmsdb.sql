...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 14, 2024 at 08:13 PM
...
(5, 876481712, 2, 'harshaaa', 6544787986, 'harsha@gmail.com', 'sss', 'Female', '03:20:00', '17:02:00', '2025-02-28', 'aa', '2024-03-14 19:10:18', NULL, NULL, NULL, '2024-03-14 19:10:18');

--
-- Triggers `tblmaidbooking`
--
DELIMITER $$
CREATE TRIGGER `check_start_date` BEFORE INSERT ON `tblmaidbooking` FOR EACH ROW BEGIN
    IF NEW.StartDate <= CURDATE() THEN
        SET @error_message = 'StartDate must be after the current date';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'check start date';
    END IF;
END
$$
DELIMITER ;
